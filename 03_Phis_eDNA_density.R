...
detect_data_phis <- detect_data_long %>% 
...
phis_full_model <- gam(detect~#s(x, bs = "ts")+
                         #s(y, bs = "ts")+
                         s(bathy)+
                         s(distShore)+
                         s(slope)+
                         s(sst),
                       data = detect_data_phis,
                       family = "binomial",
                       method = "REML")

visreg(phis_full_model)

#check output
summary(phis_full_model)
gam.check(phis_full_model)
plot(phis_full_model)
qqnorm(residuals(phis_full_model))
qqline(residuals(phis_full_model))

#density predictions
predictions =  terra::predict(data_extent, model = phis_full_model, type = "response")
...
#plot
phis_density_map <- ggplot(bs_land)+
  ...
  ggspatial::geom_spatial_point(data = detect_data_phis, 
...
save(phis_density_map, phis_full_model, file = "data products/Phis_eDNA_density.Rdata")
