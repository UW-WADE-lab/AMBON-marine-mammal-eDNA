...
detect_data_erob <- detect_data_long %>% 
...
erob_full_model <- gam(detect~#s(x, bs = "ts")+
                         #s(y, bs = "ts")+
                         s(bathy)+
                         s(distShore)+
                         s(slope)+
                         s(sst),
                       data = detect_data_erob,
                       family = "binomial",
                       method = "REML")

visreg(erob_full_model)

#check output
summary(erob_full_model)
gam.check(erob_full_model)
plot(erob_full_model)
qqnorm(residuals(erob_full_model))
qqline(residuals(erob_full_model))

#density predictions
predictions =  terra::predict(data_extent, model = erob_full_model, type = "response")
...
#plot
erob_density_map <- ggplot(bs_land)+
  ...
  ggspatial::geom_spatial_point(data = detect_data_erob, 
...
save(erob_density_map, erob_full_model, file = "data products/Erob_eDNA_density.Rdata")
